...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        ...
        //Scanning through each row and column
        for (int i=0; i<grid.length; i++) {
            for (int j=0; j<grid.length; j++) {
                //Checking for a non-empty grid
                if (grid[i][j]!='-') {
                    //checking for row conditions like:
                    //1. grid[0][0]; grid[0][1]; grid[0][2];
                    //2. grid[1][0]; grid[1][1]; grid[1][2];
                    //3. grid[2][0]; grid[1][2]; grid[2][2];
                    if (j == 0) {
                        if (grid[i][j] == grid[i][j + 1] && grid[i][j + 1] == grid[i][j + 2]) {
                            result = grid[i][j] + " wins";
                        }
                    }
                    //checking for column and diagonal conditions like:
                    //1. grid[0][0]; grid[1][0]; grid[2][0];
                    //2. grid[0][1]; grid[1][1]; grid[2][1];
                    //3. grid[0][2]; grid[1][2]; grid[2][2];
                    //4. grid[0][0]; grid[1][1]; grid[2][2];
                    //5. grid[0][2]; grid[1][1]; grid[2][0];
                    if (i == 0) {
        ...
        //If all these fail then return None
        return result;
    }
